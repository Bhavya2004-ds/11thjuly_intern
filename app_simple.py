...
try:
    from pages import dashboard, upload_data, about
    # Use the simplified analytics
    from pages import analytics_simple as analytics
    from utils.styles import load_css
    IMPORTS_SUCCESS = True
except ImportError as e:
    st.error(f"Import error: {e}")
    st.error("Please install required packages: pip install streamlit pandas numpy scikit-learn matplotlib seaborn joblib")
...
if IMPORTS_SUCCESS:
    ...
    try:
        ...
        st.info("""
        **Common Solutions:**
        1. Make sure you're using the virtual environment: `source edupredict_env/bin/activate`
        2. Install missing packages: `pip install -r requirements.txt`
        3. If using outside the environment: `pip install streamlit pandas numpy scikit-learn matplotlib seaborn joblib`
        """)
else:
    st.stop()